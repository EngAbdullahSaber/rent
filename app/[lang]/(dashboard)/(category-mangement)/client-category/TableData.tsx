...
import UpdateClientCategory from "./UpdateClientCategory";
...
const TableData = ({ flag }: { flag: any }) => {
  ...
  const getCategoryData = async () => {
    ...
    if (queryString.length > 0) {
      try {
        const res = await getFilterCategory(queryString, "client", lang);
      ...
      try {
        const res =
          page === 1
            ? await getCategory("client", lang)
            : await getCategoryPanigation(page, "client", lang);
  ...
  const SearchData = async () => {
    ...
    try {
      const res = await SearchCategory(search, "client", lang);
