...
import UpdateContactlistCategory from "./UpdateContactlistCategory";
...
const TableData = ({ flag }: { flag: any }) => {
  ...
  const getCategoryData = async () => {
    ...
    if (queryString.length > 0) {
      try {
        const res = await getFilterCategory(queryString, "contact_list", lang);
      ...
      try {
        const res =
          page === 1
            ? await getCategory("contact_list", lang)
            : await getCategoryPanigation(page, "contact_list", lang);
  ...
  const SearchData = async () => {
    ...
    try {
      const res = await SearchCategory(search, "contact_list", lang);
