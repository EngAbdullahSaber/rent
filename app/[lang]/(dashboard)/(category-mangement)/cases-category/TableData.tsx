...
import UpdateCaseCategory from "./UpdateCaseCategory";
...
const TableData = ({ flag }: { flag: any }) => {
  ...
  const getCategoryData = async () => {
    ...
    if (queryString.length > 0) {
      try {
        const res = await getFilterCategory(queryString, "cases", lang);
      ...
      try {
        const res =
          page === 1
            ? await getCategory("cases", lang)
            : await getCategoryPanigation(page, "cases", lang);
  ...
  const SearchData = async () => {
    ...
    try {
      const res = await SearchCategory(search, "cases", lang);
      ...
      cell: ({ row }) => (
        <div className="flex flex-row gap-2 items-center justify-center">
          ...
          {permission
            ...
            .permissions.some((item: any) => item.id === 78) && (
            <UpdateCaseCategory row={row} getCategoryData={getCategoryData} />
          )}
          {permission
            .find((item: any) => item.id === 75)
            .permissions.some((item: any) => item.id === 79) && (
            <DeleteButton
              id={row.original.id}
