...
import UpdateCourtCategory from "./UpdateCourtCategory";
...
const TableData = ({ flag }: { flag: any }) => {
  ...
  const getCategoryData = async () => {
    ...
    if (queryString.length > 0) {
      try {
        const res = await getFilterCategory(queryString, "courts", lang);
      ...
      try {
        const res =
          page === 1
            ? await getCategory("courts", lang)
            : await getCategoryPanigation(page, "courts", lang);
  ...
  const SearchData = async () => {
    ...
    try {
      const res = await SearchCategory(search, "courts", lang);
