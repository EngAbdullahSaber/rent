...
//py-[10px]
export const inputVariants = cva(' w-full   bg-background  border-default-300 dark:border-700  px-3 h-9   text-sm  file:border-0 file:bg-transparent file:text-sm file:font-medium  read-only:leading-9 read-only:bg-background  disabled:cursor-not-allowed disabled:opacity-50  transition duration-300 ', {
  variants: {
    color: {
      default: 'border-default-300 text-default-500 focus:outline-none focus:border-[#2684ff]  disabled:bg-default-200  placeholder:text-accent-foreground/50',
      primary: 'border-[#2684ff]  text-primary focus:outline-none focus:border-[#2684ff]  disabled:bg-primary/30 disabled:placeholder:text-primary  placeholder:text-primary/70',
...
const Input = React.forwardRef<HTMLInputElement, InputProps>(({ className, type, size, color, radius, variant, shadow, removeWrapper = false, ...props }, ref) => {
