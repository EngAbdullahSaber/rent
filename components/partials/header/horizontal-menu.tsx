...
export default function MainMenu({ trans }: { trans: any }) {
  ...
  const { Navigate, t, lang } = useTranslate();
  ...
  const onNodeUpdate = (trigger: any, itemValue: any) => {
    if (trigger && list && value === itemValue) {
      // Adjust offset calculation for RTL
      const triggerOffset = lang === 'ar' ? list.offsetWidth - (trigger.offsetLeft + trigger.offsetWidth) + trigger.offsetWidth / 6 : trigger.offsetLeft + trigger.offsetWidth / 6;

      setOffset(Math.round(triggerOffset));
    } else if (value === '') {
      setOffset(null);
    }
    return trigger;
        ...
        <NavigationMenu.List ref={setList} className='group flex list-none gap-5'>
          {menus?.map((item: any, index: number) => (
            <NavigationMenu.Item key={`item-${index}`} value={item}>
              <NavigationMenu.Trigger ref={node => onNodeUpdate(node, item)} asChild className=' flex items-center'>
              ...
              <NavigationMenu.Content className={cn('w-full rounded-md border bg-popover text-popover-foreground shadow-lg', lang === 'ar' ? 'right-0' : 'left-0')}>
                {item.child && (
                  <div className=' min-w-[200px] p-4'>
                    {item.child?.map((childItem: any, index: number) => (
                      <ListItem className='text-sm font-medium text-default-700' key={`child-${index}`} title={t(childItem.title)} href={Navigate(childItem.href)} childItem={childItem} trans={trans}>
                ...
                {item.megaMenu && (
                  <div className=''>
                    <Tabs defaultValue={item.megaMenu[0].title} onValueChange={setValue} className='inline-block p-0'>
                      <TabsList className='bg-transparent p-0 border-b-2 py-7 px-[30px] rounded-none w-full justify-start gap-10'>
                        {item.megaMenu?.map((tab: any, index: number) => (
                          <TabsTrigger
                            ...
                            <span className='text-sm font-medium text-default-700'>{tab.title}</span>
                          </TabsTrigger>
                        ))}
                      </TabsList>
                        ...
                        <TabsContent
                          ...
                          <div
                            ...
                            {tab?.child?.map((megaChild: any) => (
                              <ListItem className='mb-0 text-sm font-medium text-default-600' key={`child-${megaChild.title}`} title={megaChild.title} href={Navigate(megaChild.href)} childItem={megaChild} />
                            ))}
                          </div>
                          <div
                            ...
                            <div className='h-full w-full  text-center'>
                              <Image src={image} alt='' objectFit='cover' height={100} width={100} />
                            </div>
                          </div>
                        </TabsContent>
                      ))}
        ...
        <div className=' absolute  top-full '>
          <NavigationMenu.Viewport
            style={{
              ...
              transform:
                lang === 'ar'
                  ? `translateX(-${offset}px)` // Reverse for RTL
...
const ListItem = React.forwardRef<HTMLAnchorElement, any>(({ className, children, title, childItem, trans, ...props }, forwardedRef) => {
  // href={Navigate(href)}
  return (
    <NavigationMenu.Link asChild>
    <Link href={''} className={cn(' select-none   text-sm  text-default-700 rounded-md flex  items-center gap-2 mb-4 last:mb-0  leading-none no-underline outline-none transition-colors  hover:text-primary  focus:text-primary', className)} {...props} ref={forwardedRef}>
      <div>{children}</div>
